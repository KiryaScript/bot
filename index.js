...
client.once("ready", async () => {
    ...
    client.user.setPresence({ activities: [{ name: `Character.ai by Devorskiy`, type: ActivityType.Playing }], status: 'online' }) // Set a activity.
    ...
    if (refresh == true) {
        (async () => {
            try {
                console.log(`Запущено обновление приложения (/) commands.`)
                ...
                console.log(`Успешно перезагруженное приложение (/) commands.`)
            } catch (error) {
                console.error(error)
            }
        })();
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isCommand()) {
        ...
        try {
            ...
            await interaction.reply({ content: `Произошла ошибка. ${err}`, ephemeral: true });
        }
